...
double * StressField::getEigenvalues(const StressTensor & stressTensor)
{
...
double* StressField::getEigenvectors(const float& x, const float& y, const float& z)
{
	...
	StressTensor sTensor = this->getTensor(x, y, z);
	...
	A <<
		sTensor.sigma_x,		sTensor.tau_xy,			sTensor.tau_zx,

		sTensor.tau_xy,			sTensor.sigma_y,		sTensor.tau_zy,

		sTensor.tau_zx,			sTensor.tau_zy,			sTensor.sigma_z;
